...
func (r *Restful) SetupHandler() *operations.AppManagerAPI {
	...
	// URL: /ric/v1/xapps (POST): deploy an onboarded xApp from the helm repo (chartmuseum)
	api.XappDeployXappHandler = xapp.DeployXappHandlerFunc(
		func(params xapp.DeployXappParams) middleware.Responder {
			if params.XappDescriptor == nil || params.XappDescriptor.XappName == nil {
				appmgr.Logger.Error("No xApp descriptor or xApp name given in the deploy request")
				return xapp.NewDeployXappBadRequest()
			}
			appmgr.Logger.Info("Deploying xApp %s via helm repo", *params.XappDescriptor.XappName)
			if result, err := r.helm.Install(*params.XappDescriptor); err == nil {
				...
				appmgr.Logger.Error("xApp deploy failed: %v", err.Error())
			}
			return xapp.NewDeployXappInternalServerError()
		})

	// URL: /ric/v1/xapps/{xAppName} (DELETE): undeploy a deployed xApp
	api.XappUndeployXappHandler = xapp.UndeployXappHandlerFunc(
		func(params xapp.UndeployXappParams) middleware.Responder {
			appmgr.Logger.Info("Undeploying xApp %s", params.XAppName)
			if result, err := r.helm.Delete(params.XAppName); err == nil {
				...
				appmgr.Logger.Error("xApp undeploy failed: %v", err.Error())
			}
