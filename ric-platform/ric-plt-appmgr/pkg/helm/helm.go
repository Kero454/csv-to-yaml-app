...
func (h *Helm) GetInstallArgs(x models.XappDescriptor, cmOverride bool) (args string) {
        ...
        if x.OverrideFile != nil {
                // The REST client may send overrideFile either as a JSON string or
                // as an inline JSON object; normalize both to raw JSON bytes.
                var overrideJSON []byte
                switch v := x.OverrideFile.(type) {
                case string:
                        overrideJSON = []byte(v)
                default:
                        if b, err := json.Marshal(v); err == nil {
                                ...
                                appmgr.Logger.Info("json.Marshal(overrideFile) failed: %v", err)
                        }
                }
                if overrideYaml, err := yaml.JSONToYAML(overrideJSON); err == nil {
